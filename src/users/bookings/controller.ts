...
export default class BookingsController extends BookingsService {
  public execute = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      } else if (req.path === bookingRoutes.ONSPOTBOOKINGUPDATE) {
        if (method === RequestMethods.PATCH) {
          const { email, ticket_id, payment_id, serial_number } = req.body;
          const reqObj: any = {
            ticket_id: ticket_id,
            payment_id: payment_id,
            email: email,
          ...
          const updateResponse = await this.onSpotUpdateTicketIssued(reqObj);
          response = updateResponse;
        }
  ...
  private onSpotUpdateTicketIssued = async (
    reqObj: any
  ): Promise<IResponse> => {
    const data = await this.updateOnSpotTicketIssuedService(reqObj);
    return {
      success: true,
      message: "Ticket Issued Updated Successfully",
      data: data,
