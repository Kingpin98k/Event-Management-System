...
export default class BookingsDB {
  ...
  protected checkOnSpotTicketIssued = async (ticket_id: string): Promise<any> => {
    const query = `SELECT offline_ticket_issued FROM spotBookings WHERE ticket_id = $1 ;`;
  ...
  protected updateOnSpotOfflineTicketIssued = async (
    ticket_id: string,
    payment_id: string,
    serial_number: string,
    email: string
  ): Promise<ICreateBookingReqObj> => {
    const query = `UPDATE spotBookings AS b
    SET offline_ticket_issued = true, updated_at = current_timestamp
   
    WHERE b.email = $4
       AND b.ticket_id = $2
      AND b.payment_id = $3
      AND b.serial_number = $1
    RETURNING b.*
    ;`;
