...
export default class BookingsHelper extends BookingsDB {
	...
	protected updateOnSpotOfflineTicketIssuedHelper = async (
		reqObj: any
	) => {
	

		const checkIsAlreadyIssued: any = await this.checkOnSpotTicketIssued(
			reqObj.ticket_id
		);
		...
		const updatedBooking = await this.updateOnSpotOfflineTicketIssued(
			reqObj.ticket_id,
			reqObj.payment_id,
